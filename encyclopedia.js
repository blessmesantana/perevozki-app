const ENCYCLOPEDIA_SECTIONS = [
    ...
    },
    {
        ...
        iconMarkup: `
            <svg width="22" height="22" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M3 12.85V5.5C3 4.67157 3.67157 4 4.5 4H19.5C20.3284 4 21 4.67157 21 5.5V14.5C21 15.3284 20.3284 16 19.5 16H15" stroke="currentColor" stroke-width="1.7" stroke-linecap="round"/>
                <path d="M8 20C6.89543 20 6 20.8954 6 22V22C6 23.1046 6.89543 24 8 24C9.10457 24 10 23.1046 10 22V22C10 20.8954 9.10457 20 8 20Z" stroke="currentColor" stroke-width="1.7"/>
                <path d="M16 20C14.8954 20 14 20.8954 14 22V22C14 23.1046 14.8954 24 16 24C17.1046 24 18 23.1046 18 22V22C18 20.8954 17.1046 20 16 20Z" stroke="currentColor" stroke-width="1.7"/>
                <path d="M5.5 16.5H18.5" stroke="currentColor" stroke-width="1.7" stroke-linecap="round"/>
            </svg>
        `,
        description: 'После первого открытия приложение можно использовать без интернета. Все ваши сканирования сохраняются локально и синхронизируются автоматически, когда соединение вернётся.',
        details: [
            'После загрузки приложение кешируется на телефоне — это значит, что оно сохраняется в памяти и может открваться даже без интернета. Первый запуск требует интернета, но потом всё работает офлайн.',
            'Если нет интернета при открытии списков, приложение сразу использует кешированные данные (ждёт максимум 3 секунды вместо долгого ожидания). После этого вы можете работать как обычно.',
            'Все ваши действия остаются в локальной памяти телефона. Даже если приложение закрыть или перезагрузить устройство, ничего не потеряется — всё восстановится при следующем запуске.',
            'Когда интернет вернулся, приложение само начинает отправлять накопленные сканы. Вы увидите, как идёт синхронизация, и все данные загрузятся на сервер.',
        ],
        notes: [
            'Для первого использования нужна хотя бы одна загрузка приложения с интернетом. После этого оно работает полностью автономно.',
            'Режим офлайн поддерживает всё: сканирование QR, ввод данных вручную, удаление передач и курьеров. Все операции синхронизируются при восстановлении соединения.',
            'Приложение определяет состояние интернета автоматически. Если соединение нестабильно, оно плавно переходит между режимами без потери данных.',
        ],
    },
    {
        title: 'Настройки',
