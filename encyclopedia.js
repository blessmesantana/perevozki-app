const ENCYCLOPEDIA_SECTIONS = [
    ...
    },
    {
        ...
        `,
        description: 'Приложение работает даже без интернета! Все сканирования сохраняются локально и синхронизируются, когда соединение восстановлено.',
        details: [
            'Приложение автоматически переходит в режим офлайн, если нет соединения. Вы можете по-прежнему открывать списки курьеров, сканировать передачи и уводить их в завершённые.',
            'Все ваши действия сохраняются в локальной памяти телефона. Даже если приложение закрыть или перезагрузить телефон, никакие данные не потеряются.',
            'Когда интернет вернулся, приложение автоматически начинает отправлять накопленные сканы. Вы увидите уведомление о синхронизации, и все данные загрузятся на сервер.',
        ],
        notes: [
            'Режим офлайн работает для сканирования и удаления данных. Все операции записываются и синхронизируются после восстановления соединения.',
            'Приложение определяет состояние интернета автоматически, вы не переключаетесь вручную.',
            'Если интернет нестабилен, приложение умеет работать с перебоями и переключениями между онлайн и офлайн.',
        ],
    },
    {
        title: 'Настройки',
